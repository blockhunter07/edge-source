...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("edge-vexillum"))
        return false;
...
{
    // Convert edge-vexillum:// to edge-vexillum:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("edge-vexillum://"))
    {
        uri.replace(0, 12, "edge-vexillum:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "EDGE-VEXILLUM.lnk";
}
...
{
    return GetAutostartDir() / "edge-vexillum.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("EDGE-VEXILLUM-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  edge-vexillum-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("EDGE-VEXILLUM-Qt"));
