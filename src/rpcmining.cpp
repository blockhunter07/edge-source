...
Value checkkernel(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "EDGE-VEXILLUM is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "EDGE-VEXILLUM is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "EDGE-VEXILLUM is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "EDGE-VEXILLUM is downloading blocks...");
